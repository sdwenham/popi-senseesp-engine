...
void setup() {
  ...
  sensesp_app = (&builder)
                    ...
                    ->set_hostname("popi-sensesp-engine")
  ...
  exhaust_temp->attach([exhaust_temp]() {
    debugD("Exhaust Temp: %f", exhaust_temp->get() - 272.15);
  });

  exhaust_temp->connect_to(new Linear(1.0, 0, "/exhaustTemperature/linear"))
  ...
  alternator_temp->attach([alternator_temp]() {
    debugD("Alternator Temp: %f", alternator_temp->get() - 272.15);
  });

  alternator_temp->connect_to(new Linear(1.0, 0, "/alternatorTemperature/linear"))
  ->connect_to(new SKOutputFloat("electrical.alternators.engine.temperature",
  ...
  engine_room_temp->attach([engine_room_temp]() {
    debugD("Engine Room Temp: %f", engine_room_temp->get() - 272.15);
  });

  engine_room_temp->connect_to(new Linear(1.0, 0, "/engineRoomTemperature/linear"))
  ...
  digital_input2->connect_to(new SKOutputBool(
      ...
      new SKMetadata("",                       // No units for boolean values
                     "Badger Present")         // Value description
      ));
